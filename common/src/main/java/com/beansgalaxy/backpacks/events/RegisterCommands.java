      ...
      public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
            dispatcher.register(Commands.literal("enderdata").requires(in -> in.hasPermission(4))
                        .executes(ctx -> {
                              ...
                              return sizeE - sizeS;
                        }).then(Commands.literal("clear_player").then(Commands.argument("targets", EntityArgument.players())
                                    .executes(ctx -> {
                        ...
                        ).then(Commands.literal("list_entries").executes(ctx -> {
                              int size = ServerSave.MAPPED_ENDER_DATA.size();
                              if (size == 0) {
                                    ctx.getSource().sendSuccess(() -> Component.translatable("command.beansbackpacks.enderdata.list.fail", "No entries found.", size), false);
                                    return 1;
                              }
